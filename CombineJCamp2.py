...
initialfilename = str(input("Enter Initial DX Filename, without .DX extension, as an example to setup the wavelengths"))+".DX"
...
#        absorbance_matrix.append([float(line_list[0]),float(line_list[1])])        
        absorbance_matrix.append([float(line_list[0])])
...
for filename in dirlist:
    if filename[0:sliceendindex] == seriesname and filename[-1:-4:-1] == 'XD.':
        ...
        read_status = 'closed'
        line_index = 1
        absorbance_matrix[0].append(float(filename[-8:-3]))
        print(file)
        for line in file:
            line_list = line.split(" ")
            if old_line0 == "##XYDATA=":
